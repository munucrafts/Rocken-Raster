...
private:
	...
	glm::vec4 WorldToClip(glm::vec3& point, glm::mat4& model, StaticMesh* currentMesh);
	...
	glm::vec4 GetColorBasedOnViewMode(StaticMesh* mesh, Triangle& tri, glm::vec2& texCoords, float depthAtPixel, glm::vec3& interpNormal);
	void HandleUI();
	void RenderPixels(StaticMesh* mesh, Triangle& tri, glm::vec3& pixelA, glm::vec3& pixelB, glm::vec3& pixelC, int minX, int minY, int maxX, int maxY);
